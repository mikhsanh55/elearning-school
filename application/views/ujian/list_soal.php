		...
		<div class="row">
			<div class="col-md-12">
				...
				<?php if ($this->log_lvl != 'siswa'): ?>
					...
					<a class="btn btn-warning btn-sm tombol-kanan" href="<?php echo base_url(); ?>upload/format_soal_download-2.xlsx" ><i class="glyphicon glyphicon-download"></i> &nbsp;&nbsp;Download Format Import</a>
...
<script type="text/javascript">
	...
	function pageLoad(pg, url, search){
		$.ajax({
			...
			data :{
				...
				filter : $('#filter').val(),
				search : $('#search').val(),
				tipe_ujian_real : $('#tipe_ujian_real').val(),
				id_ujian : '<?=$ujian->id;?>'
			},
	...
	$(document).on('click','#deleted',function(){
		...
		if (y == true) {
			...
			$.ajax({
				type:'post',
				url : '<?=base_url('ujian_real/hapus_soal');?>',
				dataType : 'json',
				data : {
					id : opsi,
					link : '<?=$url_form;?>'
