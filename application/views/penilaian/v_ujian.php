...
<script type="text/javascript">
    ...
    simpan = function() {
        ...
        $.ajax({    
            type: "POST",
            url: base_url+"penilaian/simpan_satu_ujian/"+id_tes+"/"+id_penggunaan,
...
function hide() {
 ...
 simpan_akhir = function() {
        simpan();
        var id_penggunaan = $("#url3").val();
        if (confirm('Penilaian telah selesai. Anda yakin akan mengakhiri penilaian ini..?')) {
            simpan();
            $.ajax({
                type: "GET",
                url: base_url+"penilaian/simpan_akhir_ujian/"+id_tes+"/"+id_penggunaan,
                beforeSend: function() {
                    $('.ajax-loading').show();    
                },
                success: function(r) {
                    if(r.status == "ok") {
                        window.location.assign("<?php echo base_url(); ?>penilaian"); 
                    }
                }
            });
