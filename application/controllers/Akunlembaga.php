...
class Akunlembaga extends MY_Controller {
	...
	public function index(){
		...
		$this->render('akun_lembaga/list',$data);
	}
	...
	public function add(){
		...
		$this->render('akun_lembaga/add',$data);
	}
	...
	public function insert(){
		...
		if ($errors == 0) {
			...
			$kirim = $this->m_akun_lembaga->insert($data);
			if ($kirim) {
				$status = 1;
	...
	public function edit($id=0){
		$data = array(
			'instansi' => $this->m_instansi->get_all(),
			'edit' => $this->m_akun_lembaga->get_by(array('sha1(id)'=>$id))
		);
		$this->render('akun_lembaga/edit',$data);
	}
	...
	public function update(){
		...
		$cek_username = $this->validasi->check_username($post['username'],'update','akun_lembaga',$post['id']);
		$cek_email 	  = $this->validasi->check_email($post['email'],'update','akun_lembaga',$post['id']);
		...
		if ($errors == 0) {
			...
			$kirim = $this->m_akun_lembaga->update($data,array('id'=>$post['id']));
			$cek_adm = $this->db->where(array('level' => 'instansi', 'kon_id' => $post['id']))->get('m_admin')->result();
			if (count($cek_adm) > 0) {
				$data_adm = array(
					'user_id'  => $post['username'],
					'username' => $post['email']
				);
				$this->m_admin->update($data_adm, array('kon_id' => $post['id'], 'level' => 'instansi'));
			}
			if ($kirim) {
				$status = 1;
				$errors_txt['info'] = 'Berhasil Menyimpan Data';
	...
	public function aktif_non_akun(){
		...
		$kirim = $this->m_akun_lembaga->update($datas,$where);
	...
	public function page_load($pg = 1){
		...
		$paginate = $this->m_akun_lembaga->paginate($pg,$where,$limit);
		$data['paginate'] = $paginate;
		$data['paginate']['url']	= 'akunlembaga/page_load';
		$data['paginate']['search'] = 'lookup_key';
		$data['page_start'] = $paginate['counts']['from_num'];

		$this->load->view('akun_lembaga/table',$data);
	...
	public function multi_delete(){
		...
		$kirim = $this->db->where_in('id',$where)->delete('tb_akun_lembaga');

		$kirim = $this->db->where_in('kon_id',$where)->where_in('level','instansi')->delete('m_admin');
