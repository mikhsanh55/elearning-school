				...
				<div class="col-md-12">
					<div class="panel panel-info">
						<div class="panel-heading">
							<div class="tombol-kanan">
								<h2><strong>keaktifan Offline</strong></h2>
							</div>
					...
					<div class="form-group">
							...
							<div class="rs-select2 js-select-simple select--no-search" >
								<select id="kelas" class="form-control input-sm" >
									<?php foreach ($kelas as $key => $rows): ?>
										<option value="<?=$rows->id;?>"><?=$rows->nama. ' - '.$rows->nama_mapel;?></option>
									<?php endforeach ?>
								</select>
...
<script type="text/javascript">
	$(document).ready(function(){
		pageLoad(1,'keaktifan/page_load');
		...
		$(document).on('keyup','.beri-nilai',delay(function(evt){
			...
			$.ajax({
				type : 'post',
				url  : base_url + 'keaktifan/beri_nilai',
				dataType : 'json',
				data : {
					nilai : nilai,
		...
		$(document).on('keyup','.beri-nilai-aktif',delay(function(evt){
			var nilai = $(this).val();
			...
			$.ajax({
				type : 'post',
				url  : base_url + 'keaktifan/beri_nilai_aktif',
				dataType : 'json',
				data : {
					nilai : nilai,
					id_siswa : $(this).data('siswa'),
	...
	$(document).on('submit','#form-siswa',function(e){
		...
		$.ajax({		
			type: "POST",
			url: base_url+"keaktifan/m_siswa/simpan",
	...
	$(document).on('click','#deleted',function(){
		...
		if (y == true) {
			...
			$.ajax({
				type:'post',
				url : '<?=base_url('keaktifan/multi_delete');?>',
				dataType : 'json',
				data : {
					id : opsi,
				},
	...
	$(document).on('click','#edited',function(){
		...
		if (totalChecked > 1) {
			...
			window.location = base_url + 'keaktifan/edit/' + $('.checklist:checked').data('id');
		}

		
	})

	$(document).on('click','.detail-keaktifan',function(){
			window.location = base_url + 'keaktifan/lampiran_detail/' + $(this).data('id_keaktifan') + '/' + $(this).data('id_siswa');
	})
