...
$paginate['template_view'] = '#table-setting-modal';
?>
...
<nav aria-label="Page navigation" style="font-size: 12px;float: right;">
	<ul class="pagination">
		<?php if ($paginate['counts']['curr_page'] == 1): ?>
			...
			<?php else: ?>
				<li>
					<a href="javascript:void(0)" aria-label="First"  onclick="callPageLoadModal(1,'<?=$paginate['url'];?>','<?=$paginate['template_view'];?>')">
						<span aria-hidden="true"><i class="fa fa-angle-double-left"></i>First</span>
					</a>
				</li>
			<?php endif ?>
			...
			<?php if($page_start > 1): ?>
				<li><a href="javascript:void(0)" onclick="callPageLoadModal('<?=$paginate['counts']['curr_page'] - 1;?>','<?=$paginate['url'];?>','<?=$paginate['template_view'];?>')"> <i class="fa fa-angle-left"></i> Prev </a></li>
			...
			<?php for($i = $page_start; $i <= $page_end; $i++):	?>			
				...
				<?php if ($paginate['counts']['curr_page'] == $i): ?>
					...
					<?php else: ?>
						<li>
							<a href="javascript:void(0)" onclick="callPageLoadModal('<?=$i;?>','<?=$paginate['url'];?>','<?=$paginate['template_view'];?>')"><?=$i;?></a>
						</li>
					<?php endif ?>
				...
				<?php if($page_end < $count):?>
					<li><a href="javascript:void(0)"> ... </a></li>
					<li><a href="javascript:void(0)" onclick="callPageLoadModal('<?=$paginate['counts']['curr_page'] + 1;?>','<?=$paginate['url'];?>','<?=$paginate['template_view'];?>')"> Next <i class="fa fa-angle-right"></i> </a></li>
				<?php 
					endif;
				?>
				...
				<?php if ($paginate['counts']['curr_page'] == $count): ?>
					<li class = "active">
						<a href="javascript:void(0)" aria-label="Last" onclick="callPageLoadModal('<?=$count;?>','<?=$paginate['url'];?>','<?=$paginate['template_view'];?>')">
