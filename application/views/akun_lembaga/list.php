	...
	<div class="inner-box">
					...
					<div class="panel panel-info">
						<div class="panel-heading">
							<div class="tombol-kanan">
								<h2><strong>Data Akun <?=$this->transTheme->instansi;?></strong></h2>
							</div>
						</div>
					</div>
		...
		<div class="row">
			<div class="col-md-12">
				...
				<a class="btn btn-success btn-sm tombol-kanan" href="<?=base_url('akunlembaga/add');?>"><i class="fa fa-user-plus"></i> &nbsp;Tambah</a>
...
<script type="text/javascript">
	$(document).ready(function(){
		pageLoad(1,'akunlembaga/page_load');
	...
	$(document).on('click','#deleted',function(){
		...
		if (y == true) {
			...
			$.ajax({
				type:'post',
				url : '<?=base_url('akunlembaga/multi_delete');?>',
				dataType : 'json',
				data : {
					id : opsi,
				},
	...
	$(document).on('click','#edited',function(){
		...
		if (totalChecked > 1) {
			...
			window.location = base_url + 'akunlembaga/edit/' + $('.checklist:checked').data('id');			
		}
	...
	$(document).on('click','.buat-pass',function(){
		...
		if (y == true) {
			...
			$.ajax({
				type:'post',
				url : '<?=base_url('akunlembaga/buatkan_password');?>',
				dataType : 'json',
				data : {
					id : $(this).data('id'),
				},
	...
	$(document).on('click','.reset-pass',function(){
		...
		if (y == true) {
			...
			$.ajax({
				type:'post',
				url : '<?=base_url('akunlembaga/reset_password');?>',
				dataType : 'json',
				data : {
					id : $(this).data('id'),
				},
	...
	$(document).on('click','.aktif-non-akun',function(){
		...
		if (y == true) {
			...
			$.ajax({
				type:'post',
				url : '<?=base_url('akunlembaga/aktif_non_akun');?>',
				dataType : 'json',
				data : {
					id : $(this).data('id'),
